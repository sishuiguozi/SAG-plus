...
async def test_incremental_processor_passes_chunk_settings_to_zleap(monkeypatch):
    ...
    class FakeLoader:
        ...
        async def load(self, config):
            ...
            seen["embedding_batch_size"] = config.embedding_batch_size
            seen["es_bulk_index_size"] = config.es_bulk_index_size
...
@pytest.mark.asyncio
async def test_reprocess_failed_document_with_existing_queued_job_restores_pending_status():
    from sag_api.core.db import SessionLocal, init_db
    from sag_api.db.models import Document, Job, Source
    from sag_api.enums import DocumentStatus, JobStatus, JobType
    ...
    await init_db()
    async with SessionLocal() as session:
        source = Source(name="retry-source", sag_source_config_id="retry-source-config")
        session.add(source)
        await session.flush()
        document = Document(
            source_id=source.id,
        ...
        queue = FakeQueue()
        returned_job = await reprocess_document(
            session,
            source,
            document.id,
            job_queue=queue,
        ...
        assert returned_job.status == JobStatus.QUEUED
