    ...
    try:
        async with app.router.lifespan_context(app):
            ...
            async with httpx.AsyncClient(transport=transport, base_url="http://t") as client:
                ...
                body = response.json()
                config = body["config"]
                # effective_model_config 会包含系统/高级字段（向量、磁盘、宇宙预算等）；
                # 这里只校验快捷预设必须写出的核心字段，并额外确认新字段一并回传。
                expected_subset = {
                    "llm_provider": "openai",
                ...
                for key, value in expected_subset.items():
                    assert config[key] == value, f"{key}: {config.get(key)!r} != {value!r}"
                for key in (
                    "lancedb_ann_enabled",
                    ...
                    assert key in config, f"effective config missing {key}"
