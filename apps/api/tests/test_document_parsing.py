...
async def _noop_assert_public_host(_host: str, _port: int) -> None:
    return None
...
@pytest.mark.asyncio
async def test_official_mineru_upload_poll_and_download_zip(tmp_path, monkeypatch):
    source = tmp_path / "paper.pdf"
    source.write_bytes(b"%PDF-fake")
    _FakeAsyncClient.reset(
        [
            _response(
                json={
                    ...
                    "data": {
                        "batch_id": "batch-123",
                        "file_urls": ["https://mineru.oss-cn-shanghai.aliyuncs.com/upload.pdf"],
                    },
                },
                url="https://mineru.net/api/v4/file-urls/batch",
            ),
            _response(json={"ok": True}, url="https://mineru.oss-cn-shanghai.aliyuncs.com/upload.pdf"),
            _response(
                json={"code": 0, "data": {"batch_id": "batch-123", "extract_result": [{"state": "running"}]}},
            ...
            _response(
                content=_result_zip("# Official\n\nMinerU result"),
                content_type="application/zip",
                url="https://example.com/mineru/result.zip",
            ),
        ]
    )
    monkeypatch.setattr(httpx, "AsyncClient", _FakeAsyncClient)
    monkeypatch.setattr(mineru_module, "_assert_public_host", _noop_assert_public_host)
    states: list[dict[str, Any]] = []
    client = MinerUClient(
        _settings(
    ...
    assert markdown == "# Official\n\nMinerU result\n"
    assert any(state.get("batch_id") == "batch-123" for state in states)
    assert any(state.get("uploaded") is True for state in states)
    assert [call[0] for call in _FakeAsyncClient.calls] == [
        "POST",
        "PUT",
        "GET",
    ...
    assert _FakeAsyncClient.calls[0][2]["json"]["model_version"] == "vlm"
    assert _FakeAsyncClient.calls[2][1] == "https://mineru.net/api/v4/extract-results/batch/batch-123"
    assert "Authorization" not in _FakeAsyncClient.calls[-1][2]


@pytest.mark.asyncio
async def test_official_mineru_retry_reuses_uploaded_batch(tmp_path, monkeypatch):
    source = tmp_path / "paper.pdf"
    source.write_bytes(b"%PDF-fake")
    _FakeAsyncClient.reset(
        [
            ...
            _response(
                content=_result_zip("# Already uploaded"),
                content_type="application/zip",
                url="https://example.com/mineru/result.zip",
            ),
        ]
    ...
    markdown = await client.parse(
        ...
        state={
            "batch_id": "batch-123",
            "upload_url": "https://mineru.oss-cn-shanghai.aliyuncs.com/upload.pdf",
            "uploaded": True,
        },
    )

    assert markdown == "# Already uploaded\n"
    assert [call[0] for call in _FakeAsyncClient.calls] == ["GET", "DOWNLOAD"]
