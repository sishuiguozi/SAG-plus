...
@pytest.mark.asyncio
async def test_universe_interactive_graph_busy_status_boundary():
    from sag_api.core.db import SessionLocal
    from sag_api.db.models import Document, Source, VectorWriteJob
    from sag_api.enums import DocumentStatus
    from sag_api.main import app
    from sag_api.services.universe_service import _source_interactive_graph_is_busy
    ...
    async with app.router.lifespan_context(app):
        async with SessionLocal() as session:
            source = Source(
                name="探索状态边界",
                description="",
                sag_source_config_id=f"src_universe_{uuid.uuid4().hex[:16]}",
            )
            session.add(source)
            await session.flush()
            stable_docs = [
                Document(
                    ...
                    size_bytes=1,
                    storage_path="/tmp/universe-ready.md",
                ...
                Document(
                    ...
                    content_type="text/markdown",
                    size_bytes=1,
                    storage_path="/tmp/universe-paused.md",
                ...
                Document(
                    ...
                    content_type="text/markdown",
                    size_bytes=1,
                    storage_path="/tmp/universe-failed.md",
                    status=DocumentStatus.FAILED,
                ),
            ]
            session.add_all(stable_docs)
            session.add(
                VectorWriteJob(
            ...
            assert await _source_interactive_graph_is_busy(session, source) is False
            ...
            blocking = Document(
                ...
                content_type="text/markdown",
                size_bytes=1,
                storage_path="/tmp/universe-pending.md",
                status=DocumentStatus.PENDING,
            )
            session.add(blocking)
            await session.commit()
            assert await _source_interactive_graph_is_busy(session, source) is True
...
@pytest.mark.asyncio
async def test_universe_timeline_uses_cache_and_singleflight():
    from sag_api.core.db import SessionLocal
    from sag_api.db.models import Document, Source
    from sag_api.enums import DocumentStatus
    from sag_api.main import app
    from sag_api.sag.dto import UniverseTimelineBundleInfo, UniverseTimelineInfo
    from sag_api.services.universe_service import universe_timeline

    class TimelineEngine:
        calls = 0

        async def universe_timeline(self, source_config_id, **_kwargs):
            self.calls += 1
    ...
    async with app.router.lifespan_context(app):
        async with SessionLocal() as session:
            source = Source(
                name="探索时间轴缓存",
                description="",
                sag_source_config_id=f"src_timeline_{uuid.uuid4().hex[:16]}",
                document_count=1,
                chunk_count=1,
                event_count=1,
            )
            ...
            session.add(
                Document(
                    ...
                    content_type="text/markdown",
                    size_bytes=1,
                    storage_path="/tmp/timeline-ready.md",
                    status=DocumentStatus.READY,
                    chunk_count=1,
                    event_count=1,
                    sag_source_id="timeline-document",
                )
            )
            await session.commit()
            source_id = source.id

        async def load_once() -> dict:
            async with SessionLocal() as session:
                return await universe_timeline(
                    session,
                    engine,
        ...
        assert first["bundles"][0]["event"]["label"] == "缓存时间轴"
        assert second["bundles"][0]["event"]["label"] == "缓存时间轴"
        assert engine.calls == 1

        third = await load_once()
        assert third["bundles"][0]["event"]["label"] == "缓存时间轴"
        assert engine.calls == 1
    ...
    try:
        ...
        async with app.router.lifespan_context(app):
            ...
            async with httpx.AsyncClient(transport=transport, base_url="http://t") as client:
                ...
                expand_calls = engine.expand_calls
                expanded_again = await client.post(
                    "/api/v1/universe/expand",
                    headers=headers,
                    json={
                        "epoch": 8,
                ...
                assert expanded_again.status_code == 200, expanded_again.text
                assert expanded_again.json()["nodes"][0]["id"] == entity_id
                assert engine.expand_calls == expand_calls
                ...
                detail_calls = engine.detail_calls
                detail_again = await client.get(
                    f"/api/v1/universe/nodes/event/{event_id}?source_id={source_id}",
                    headers=headers,
                )
                assert detail_again.status_code == 200, detail_again.text
                assert detail_again.json()["evidence"]["content"] == "所有星点都能回到这段真实原文。"
                assert engine.detail_calls == detail_calls
