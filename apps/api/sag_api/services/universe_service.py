...
_UNIVERSE_READ_TASKS: dict[asyncio.AbstractEventLoop, dict[str, asyncio.Task[Any]]] = {}
_UNIVERSE_READ_TASK_LOCKS: dict[asyncio.AbstractEventLoop, asyncio.Lock] = {}
_UNIVERSE_CACHE_SCHEMA_VERSION = 1
...
def _universe_read_task_lock() -> asyncio.Lock:
    loop = asyncio.get_running_loop()
    return _UNIVERSE_READ_TASK_LOCKS.setdefault(loop, asyncio.Lock())


def _universe_read_tasks() -> dict[str, asyncio.Task[Any]]:
    loop = asyncio.get_running_loop()
    return _UNIVERSE_READ_TASKS.setdefault(loop, {})


def _cache_request_hash(payload: dict[str, Any]) -> str:
    raw = json.dumps(
        payload,
        ensure_ascii=True,
        allow_nan=False,
...
def _universe_cache_key(kind: str, payload: dict[str, Any]) -> tuple[str, str]:
    request_hash = _cache_request_hash(
        {
            "schema": _UNIVERSE_CACHE_SCHEMA_VERSION,
            "kind": kind,
            **payload,
        }
    )
    return f"{kind}:v{_UNIVERSE_CACHE_SCHEMA_VERSION}:{request_hash}", request_hash
...
async def _load_universe_cache(
    ...
    *,
    ...
    conditions = [
        UniverseGraphCache.source_id == source.id,
        UniverseGraphCache.kind == kind,
        UniverseGraphCache.cache_key == cache_key,
    ]
    if revision is not None:
        conditions.append(UniverseGraphCache.revision == revision)
    cache = await session.scalar(
        select(UniverseGraphCache)
        .where(*conditions)
        .order_by(UniverseGraphCache.updated_at.desc(), UniverseGraphCache.id.desc())
        .limit(1)
    )
...
async def _save_universe_cache(
    session: AsyncSession,
    source: Source,
    *,
    kind: str,
    ...
    cache = await session.scalar(
        select(UniverseGraphCache).where(
            UniverseGraphCache.source_id == source_id,
            UniverseGraphCache.cache_key == cache_key,
        )
    )
    encoded = jsonable_encoder(payload)
    def apply(cache_row: UniverseGraphCache) -> None:
        cache_row.source_config_id = source_config_id
        cache_row.kind = kind
        cache_row.revision = revision
        cache_row.request_hash = request_hash
        cache_row.payload = encoded
        cache_row.built_at = datetime.now(UTC)
        cache_row.error = None

    if cache is None:
        cache = UniverseGraphCache(
            source_id=source_id,
...
async def _run_universe_singleflight(
    key: str,
    factory,
) -> Any:
    tasks = _universe_read_tasks()
    lock = _universe_read_task_lock()
    async with lock:
        task = tasks.get(key)
...
async def _source_interactive_graph_is_busy(session: AsyncSession, source: Source) -> bool:
    """Whether interactive universe reads should stand down for ingestion.

    Paused/ready/failed documents are stable enough to explore. Pending/loading/
    extracting documents or active process jobs mean the source is still being
    written and interactive graph reads should avoid competing for document
    ingestion resources. Durable event-vector writes continue in the background
    and should not block structural graph exploration.
    """
...
async def _ensure_source_interactive_graph_ready(session: AsyncSession, source: Source) -> None:
    if await _source_interactive_graph_is_busy(session, source):
        raise ServiceUnavailableError("入库仍在进行，暂不加载探索图谱，避免挤占入库资源。")
...
async def universe_expand(
    ...
    *,
    ...
    cache_key, request_hash = _universe_cache_key(
        "expand",
        {
            "user_id": user_id,
            "source_id": source.id,
    ...
    cached = await _load_universe_cache(
        session,
        source,
        kind="expand",
        cache_key=cache_key,
    ...
    if cached is not None:
        return cached
    if await _source_interactive_graph_is_busy(session, source):
        stale = await _load_universe_cache(
            session,
            source,
            kind="expand",
            cache_key=cache_key,
    ...
    async def _fetch_expansion():
        return await engine_manager.universe_expand(
            source.sag_source_config_id,
            node_kind,
            node_id,
            source=source,
    ...
    try:
        # Cold reads on a large SQLite graph may still need a few seconds to
        # hydrate entity counts. Successful pages are materialized afterward.
        async with asyncio.timeout(20.0):
            expansion = await _run_universe_singleflight(
                f"universe:{source.id}:{cache_key}",
                _fetch_expansion,
            )
...
async def universe_timeline(
    ...
    *,
    ...
    async def _fetch_timeline():
        return await engine_manager.universe_timeline(
            source.sag_source_config_id,
            source=source,
            limit=bounded_limit,
            entity_limit=settings.universe_event_entity_limit,
    ...
    try:
        async with asyncio.timeout(8.0):
            page = await _run_universe_singleflight(
                f"universe:{source.id}:{cache_key}",
                _fetch_timeline,
            )
    except TimeoutError as error:
...
async def universe_node_detail(
    ...
    *,
    ...
    async def _fetch_detail():
        return await engine_manager.universe_node_detail(
            source.sag_source_config_id,
            node_kind,
            node_id,
            source=source,
    ...
    try:
        async with asyncio.timeout(8.0):
            detail = await _run_universe_singleflight(
                f"universe:{source.id}:{cache_key}",
                _fetch_detail,
            )
    except TimeoutError as error:
