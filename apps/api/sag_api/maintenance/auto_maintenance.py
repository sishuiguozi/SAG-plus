...
def _run_script(script: str, args: list[str]) -> int:
    # 直接调用包内维护模块（与 scripts/ 命令行入口共用同一份逻辑）。
    from sag_api.maintenance.cleanup_lancedb_old_versions import run as run_cleanup
    from sag_api.maintenance.optimize_lancedb_table import run as run_optimize

    print(f"[auto-maintenance] run {script} {' '.join(args)}")
    try:
        if script == "optimize_lancedb_table.py":
            return int(run_optimize(args))
        if script == "cleanup_lancedb_old_versions.py":
            return int(run_cleanup(args))
