...
describe("source graph API document scope", () => {
  ...
  it("allows separate graph budgets for documents, events, and entities", async () => {
    const fetchMock = vi.fn().mockResolvedValue(graphResponse());
    vi.stubGlobal("fetch", fetchMock);
    ...
    const url = new URL(String(fetchMock.mock.calls[0][0]), "http://localhost");
    expect(url.searchParams.get("document_limit")).toBe("240");
    expect(url.searchParams.get("event_limit")).toBe("220");
    expect(url.searchParams.get("entity_limit")).toBe("120");
  });
