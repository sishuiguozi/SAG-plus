      ...
      <div className="flex flex-wrap gap-1.5">
        {questions.map((q, i) => (
          <button
            ...
            type="button"
            onClick={() => onPick(q)}
            className="rounded-full border bg-card px-2.5 py-1 text-left text-xs text-muted-foreground shadow-soft transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
          >
            {q}
          </button>
        ))}
        ...
        >
          ...
          {activeView === "graph" && search.result.events.length > 0 ? (
            ...
            <>
              ...
              {search.result
                && (search.result.sections.length > 0 || search.result.events.length > 0) && (
                <RelatedQuestions
                  query={search.result.query}
                  onPick={(q) => {
                    search.setQuery(q);
                    void executeSearch({ query: q, saveExploration: false });
                  }}
                />
              )}
            </>
