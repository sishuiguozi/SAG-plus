...
export function SearchProvider({
  ...
  // E2：进入检索页时若 URL 带 ?q=，自动恢复并执行该查询
  const restoredUrlQueryRef = React.useRef(false);
  React.useEffect(() => {
    if (restoredUrlQueryRef.current) return;
    const q = new URLSearchParams(window.location.search).get("q")?.trim() ?? "";
    if (!q) return;
    restoredUrlQueryRef.current = true;
    void run({ query: q, intent: "replace" });
  ...
  const run = React.useCallback(async (options: SearchRunOptions = {}) => {
    ...
    const commitResult = (result: SearchResponse) => {
      ...
      // E2：把已提交的查询同步到 URL（不触发导航），刷新/分享可恢复
      try {
