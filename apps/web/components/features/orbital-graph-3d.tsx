...
import { orbitalGraphPerformanceProfile } from "@/components/features/graph-performance";
...
function addBackgroundPoints(scene: THREE.Scene, pointCount: number) {
  let seed = 24681357;
  const random = () => {
...
export function OrbitalEventEntityGraph({
  ...
  const performanceProfile = React.useMemo(
    () =>
      orbitalGraphPerformanceProfile({
        ...
        relationCount: slice.relations.length,
      }),
    [slice],
  );
  ...
  React.useEffect(() => {
    ...
    renderer.setPixelRatio(
      Math.min(window.devicePixelRatio, performanceProfile.pixelRatioCap),
    );
    ...
    addBackgroundPoints(scene, performanceProfile.backgroundPointCount);
    ...
    if (eventSurfaceMode === "plates") {
      ...
      const plateSurface = buildEventPlateSurface(
        slice,
        layout,
        performanceProfile.plateGeometryDetail,
      );
    ...
    slice.relations.forEach((relation, index) => {
      ...
      const points = curve.getPoints(performanceProfile.edgeCurveSegments);
    ...
    const selectNode = (id: string) => {
      ...
      if (kind === "event") {
        ...
        if (!value) return;
        const clusterKey = entityClusterKeyFromId(value.id);
        if (clusterKey) {
          setSelection(null);
    ...
    const render = (time: number) => {
      ...
      if (
        performanceProfile.minimumFrameIntervalMs > 0 &&
        lastRenderedAt > 0 &&
        time - lastRenderedAt < performanceProfile.minimumFrameIntervalMs
      ) {
      ...
      <div className="pointer-events-none absolute left-3 top-24 z-10 max-w-[calc(100%-1.5rem)] rounded-md border border-white/10 bg-[#0b1020]/85 px-2.5 py-2 text-white/70 shadow-lg backdrop-blur-md sm:top-3">
        <div className="flex flex-wrap gap-x-3 gap-y-1.5">
          ...
          {clustered.clusteredEntityCount > 0 && (
            <span className="text-[10px] text-[#9ab0ff]">
              {t("clusterHint", {
