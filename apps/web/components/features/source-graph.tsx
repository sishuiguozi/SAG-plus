...
import { progressiveSourceGraphBudgets } from "@/lib/source-graph-progressive";
...
import { sourceGraph2dPerformanceProfile } from "@/components/features/graph-performance";
...
interface GraphNodeData extends Record<string, unknown> {
  ...
  entity?: Entity;
  /** 当前切片中该事件是否至少有一条可见关系边（SAG-OPT-603） */
  hasEdge?: boolean;
}
  ...
  return (
    ...
    >
      ...
      <div className="px-2.5 py-2">
        ...
        {node.kind === "event" && !node.hasEdge && (
          <div
            className={cn(
              "mt-1.5 truncate rounded px-1 py-0.5 text-[9px] font-medium",
              (node.event?.relation_count ?? 0) > 0
...
const GRAPH_LIMIT_STORAGE_KEY = "sag:source-graph-limit:v2";
const SOURCE_GRAPH_BUDGET = {
  "2d": { documents: 300, events: 300, entities: 180 },
  "3d": { documents: 240, events: 220, entities: 120 },
} as const;
const GRAPH_LIMIT_OPTIONS = [
  100, 200, 300, 600, 1_000, 2_000,
] as const;
  ...
  return [
    ...slice.events.map((event) => ({
      ...
      data: {
        ...
        event,
        hasEdge: slice.relations.some((relation) => relation.eventId === event.id),
...
function buildNetwork(
  ...
  const performanceProfile = sourceGraph2dPerformanceProfile(simNodes.length);
  // Force simulation is quadratic and blocks React's main thread. Dense graph
  // slices use the deterministic radial seed directly; smaller slices retain
  // force layout with an adaptive, bounded simulation budget.
  if (!performanceProfile.useForceLayout) {
    return {
      nodes: radialNodes,
      edges: makeEdges(slice, radialPositions, layout),
    };
  ...
  const simulation = forceSimulation<SimNode>(simNodes)
    ...
    .force(
      ...
      forceCollide<SimNode>((node) => collisionRadius(node.kind))
        .strength(0.96)
        .iterations(performanceProfile.collisionIterations),
    )
    .force(
      "x",
  ...
  for (
    let index = 0;
    index < performanceProfile.simulationTicks;
    index += 1
  ) {
    simulation.tick();
  }
...
export function EventEntityGraph({
  ...
  const [expandedClusterKeys, setExpandedClusterKeys] = React.useState<
    Set<string>
  >(new Set());
  ...
  const rawSlice = React.useMemo(() => sliceEventEntityGraph(graph), [graph]);
  const clustered = React.useMemo(
    () =>
      clusterEventEntityGraph(rawSlice, expandedClusterKeys, {
        uncategorizedLabel: t("uncategorized"),
      }),
    [expandedClusterKeys, rawSlice, t],
  );
  const slice = clustered.slice;
  const labels = React.useMemo<GraphLabels>(
    () => ({
      ...
      relationNotLoaded: t("relationNotLoaded"),
    }),
    [t],
  );
    ...
    <GraphCanvas
      ...
      legend={
        ...
        >
          ...
          {clustered.clusteredEntityCount > 0 && (
            <div className="mt-1 text-[10px] text-violet-700 dark:text-violet-300">
              {t("clusterHint", {
                count: clustered.clusteredEntityCount,
                clusters: clustered.clusters.size,
              })}
            </div>
          )}
        </GraphLegend>
      ...
      toolbarActions={
        <>
          {expandedClusterKeys.size > 0 && (
            <Button
              variant="outline"
              size="sm"
              className="h-8 bg-card/95 px-2 text-xs shadow-soft backdrop-blur-sm"
              onClick={() => {
                setSelection(null);
                setExpandedClusterKeys(new Set());
              }}
              title={t("collapseClusters")}
            >
              {t("collapseClusters")}
            </Button>
          )}
      ...
      onNodeClick={(_event, node) => {
        ...
        if (data.kind === "event" && data.event) {
          ...
          const clusterKey = entityClusterKeyFromId(data.entity.id);
          if (clusterKey) {
            setSelection(null);
...
export function SourceGraph({
  ...
  const graphBudget = SOURCE_GRAPH_BUDGET[mode];
  const documentLimit = Math.min(graphLimit, graphBudget.documents);
  const eventLimit = Math.min(graphLimit, graphBudget.events);
  const entityLimit = Math.min(graphLimit, graphBudget.entities);
  ...
  React.useEffect(() => {
    ...
    // Do not abort a request that is already running: it may still populate
    // the server cache. Once this view becomes stale, however, do not start
    // later expansion stages, so the currently visible source gets priority.
    let visible = true;
    const stages = progressiveSourceGraphBudgets({
      documentLimit,
      eventLimit,
      entityLimit,
    });
    setError("");
    setRefreshing(true);
    setLoadProgress({ completed: 0, total: stages.length });
    ...
    void (async () => {
      try {
        for (let index = 0; index < stages.length; index += 1) {
          ...
          const response = await api.getSourceGraph(source.id, {
            limit: graphLimit,
            documentLimit: stage.documentLimit,
            eventLimit: stage.eventLimit,
            entityLimit: stage.entityLimit,
            documentIds,
          });
          if (!visible) return;
          setGraph(response);
  ...
  const toolbarActions = (
    <>
      ...
      <Button
        variant="outline"
        size={refreshing ? "sm" : "icon"}
        className={cn(
          "h-8 bg-card/95 shadow-soft backdrop-blur-sm",
          refreshing ? "gap-1.5 px-2 text-xs" : "w-8",
        )}
        onClick={() => setRefreshVersion((value) => value + 1)}
        disabled={refreshing}
        aria-label={
  ...
  const sharedProps = {
    graph,
    refreshKey: `${source.id}-${refreshKey}-${refreshVersion}-${documentLimit}-${eventLimit}-${entityLimit}-${documentScopeKey}`,
    toolbarActions,
    onOpenEvent,
    emptyTitle:
