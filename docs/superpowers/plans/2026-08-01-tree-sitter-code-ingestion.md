...
- [ ] **步骤 4：实现只暴露代码配置的 schema/service/API**
...
- [ ] **步骤 4：实现 replacement payload 和发布边界**
...
- [ ] 306 语言资源、精确版本、后台全量下载、进度/暂停/继续/修复、约 500 MB 说明均有实现与测试。
- [ ] 单文件与代码文件夹的路由差异有参数化测试；Markdown/PDF/Office/普通文本无回归。
- [ ] 递归选择、根目录名、相对路径、`.gitignore`、敏感/生成/二进制排除均有前后端双重校验。
- [ ] 增量 new/changed/unchanged/rejected 完整，本地缺失不删除。
- [ ] Document 三字段与两个索引通过幂等迁移验证。
- [ ] 稳定符号 ID 不含行号；两级父子、长函数 statement 子分块、祖先路径完整。
- [ ] off/comments/all 是每知识库配置，旧库默认 comments；comments 只把注释文本送入 LLM。
- [ ] 更新过程旧版本持续可用；失败回滚；发布后按 hash 过滤旧块；清理可重试。
- [ ] 代码检索返回父紧凑上下文 + 精确子源码，通用父子增强行为不变。
- [ ] `.ipynb` 明确不在本期；未引入完整代码图数据库。
- [ ] 所有新增 API、类型、中英文案、README 和架构补丁文档一致，无未完成标记和失效启动方式。
