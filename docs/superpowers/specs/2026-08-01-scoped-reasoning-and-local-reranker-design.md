...
首发提供三种向量模型和三种重排模型，用于中文、英文、代码和混合知识库的召回与最终候选排序。
...
- 本地重排只处理已召回的有限候选（默认最多 20 条）；不改变 BM25、父子分块或索引写入。
...
启用开关、候选数及刷新按钮。所有模型默认不下载；切换向量模型时页面会明确提示重新向量化
对应知识库。
...
重排模型中分别下载、启用一种。切换向量模型后，系统提示用户重新向量化相应知识库。启用本地
重排后，知识库检索先完成现有向量/BM25/规则融合，再以本地分数稳定重排有限候选。任何本地
模型前置条件或推理失败都不会中断搜索。
