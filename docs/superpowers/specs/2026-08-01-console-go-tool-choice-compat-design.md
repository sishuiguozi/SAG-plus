...
另新增系统配置 `llm_reasoning_history_compat`，默认值为 `auto`，设置页名称为“推理历史兼容”，支持三档：

| 值 | 设置页名称 | 行为 |
| --- | --- | --- |
| `auto` | 自动（推荐） | 仅当模型、供应商或基础 URL 的路由文本包含 `deepseek` 时补齐历史字段 |
| `always` | 始终启用 | 对所有模型路由补齐历史字段，支持模型别名和未被自动识别的兼容接口 |
| `off` | 关闭 | 不补齐任何历史字段 |

该设置与四档工具调用策略相互独立，保存后同时作用于流式和非流式请求。
...
- `llm_reasoning_history_compat="always"`，或其值为 `auto` 且模型、供应商或基础 URL 的路由文本包含 `deepseek`；
...
推理内容原样保留，调用方传入的请求与消息对象不得被修改。`off` 模式、`auto` 下未识别为
DeepSeek 的路由、当前仍关闭思考的请求以及非 assistant 消息不做处理。
...
5. 当前请求保留思考时，按“推理历史兼容”的三档配置决定是否补齐历史 assistant 消息。
6. 已有入库抽取、实体事件抽取和 LLM 重排作用域继续独立关闭思考，不受四档聊天策略削弱。

所有处理必须是单次请求级的，不得修改全局模型请求对象或泄漏到下一轮。用户显式提供的其他 `extra_body` 字段必须保留，只合并本策略需要的字段。

## 后端改动范围

- 核心配置：保留四值工具策略，并新增三值推理历史兼容字段与推荐默认值。
- 系统设置 schema：校验并向前端输出两个字段。
- 设置服务：读写、推荐值和重置默认值。
...
- 模型配置类型增加工具调用策略和推理历史兼容模式。
- 模型配置表单增加四档工具策略、三档历史兼容选择、说明和推荐标记。
- 中英文文案同步补齐。
...
- 历史兼容 `always` 对模型别名和非 DeepSeek 路由也补齐字段。
- 历史兼容 `off` 不补齐字段；`auto` 仅匹配 DeepSeek 路由文本。
- 已有 `reasoning_content` 保持不变，原始请求和嵌套消息对象不被修改。
- 自动模式下的非 DeepSeek 路由或关闭思考的请求不增加该历史字段。
- 入库抽取和重排已有关闭思考作用域在任意聊天策略下继续生效。
- 流式与非流式请求使用相同策略。
- 系统设置 API 能保存、读取和重置四档值。
- 系统设置 API 能保存、读取和重置三档推理历史兼容值。
...
- 模型配置可选择、保存并恢复四种策略。
- 模型配置可选择、保存并恢复自动、始终启用和关闭三种推理历史兼容模式。
...
- 模型使用别名时，用户选择“始终启用”也能完成工具后的思考回答。
- “你好”等无需工具的直接回答行为保持正常。
...
- “始终启用”可能向不认识该字段的接口发送额外消息字段，因此默认使用“自动”，遇到兼容问题时
  用户可切换为“关闭”。
